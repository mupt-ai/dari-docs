...
Managed runs default to three tasks per run and three active runs per account at a time. Admins can override those limits for individual users. Tester sessions are started with the Dari session-batch API; optimize runs start the editor after tester feedback is complete.
