...
func (s *Server) maxTasksPerRun() int {
	if s.cfg.MaxTasksPerRun > 0 {
		return s.cfg.MaxTasksPerRun
	}
	return int(managedMaxTasksPerRun)
}

func (s *Server) managedRunLimitsForUser(ctx context.Context, userID string) (managedRunLimits, error) {
	limits := managedRunLimits{
		MaxTasksPerRun:       s.maxTasksPerRun(),
...
`, userID).Scan(&taskOverride, &activeOverride)
	...
	applyManagedRunLimitOverrides(&limits, taskOverride, activeOverride)
	return limits, nil
}

func applyManagedRunLimitOverrides(limits *managedRunLimits, taskOverride, activeOverride sql.NullInt64) {
...
WHERE id=$1
FOR UPDATE
`, userID).Scan(&taskOverride, &activeOverride); err != nil {
		return err
	}
	limits := managedRunLimits{
		MaxTasksPerRun:       s.maxTasksPerRun(),
