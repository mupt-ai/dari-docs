...
`, pattern, limit)
	...
	for rows.Next() {
		...
		s.applyAdminUserLimitDefaults(&user)
...
func (s *Server) handleAdminUserLimits(w http.ResponseWriter, r *http.Request, userID string) {
	...
	if err := validateAdminRunLimitOverride("max_tasks_per_run", maxTasks); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	if err := validateAdminRunLimitOverride("max_active_runs_per_user", maxActive); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func (s *Server) getAdminUserSummary(ctx context.Context, userID string) (adminUserSummary, error) {
	summary, err := scanAdminUserSummary(s.db.QueryRow(ctx, adminUserSummarySelect+`WHERE u.id=$1`, userID))
	if err != nil {
		return adminUserSummary{}, err
	}
	s.applyAdminUserLimitDefaults(&summary)
	return summary, nil
}
...
WHERE id=$1
RETURNING id
`, userID, adminNullableInt(maxTasks), adminNullableInt(maxActive)).Scan(&updatedID)
	if err != nil {
		return adminUserSummary{}, err
	}
...
func (s *Server) applyAdminUserLimitDefaults(summary *adminUserSummary) {
	summary.EffectiveMaxTasksPerRun = s.maxTasksPerRun()
	if summary.MaxTasksPerRunOverride != nil {
		summary.EffectiveMaxTasksPerRun = *summary.MaxTasksPerRunOverride
	}
	summary.EffectiveMaxActiveRunsPerUser = s.maxActiveRunsPerUser()
	if summary.MaxActiveRunsPerUserOverride != nil {
		summary.EffectiveMaxActiveRunsPerUser = *summary.MaxActiveRunsPerUserOverride
	}
}
...
func validateAdminRunLimitOverride(name string, value *int) error {
	if value == nil {
		return nil
	}
	if *value < 0 {
		return fmt.Errorf("%s must be null or a non-negative integer", name)
	}
	return nil
}

func adminNullableInt(value *int) any {
	if value == nil {
		return nil
	}
	return *value
