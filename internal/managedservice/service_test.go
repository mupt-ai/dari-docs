...
func TestHandleRunConfigReturnsLaunchPricingAndLimits(t *testing.T) {
	...
	if strings.Join(got.DefaultFeedbackLLMIDs, ",") != "claude-haiku-4-5,claude-sonnet-4-7,claude-opus-4-6" {
		t.Fatalf("default_feedback_llm_ids = %#v", got.DefaultFeedbackLLMIDs)
	}
	if strings.Join(got.AllowedLLMIDs, ",") != "claude-haiku-4-5,claude-sonnet-4-7,claude-opus-4-6,gpt-5-mini,gpt-5.1,gpt-5.5" {
...
func TestReserveRunStoresConfiguredHostedAgents(t *testing.T) {
	...
	if err := s.reserveRun(ctx, userID, runID, "check", []byte(`["task"]`), []byte(`["claude-haiku-4-5","claude-opus-4-6"]`), "claude-opus-4-6", runSourceCLI, result, 150, false, []byte(`[]`), nil, nil); err != nil {
		t.Fatal(err)
	}
...
`, runID).Scan(&testerAgentID, &testerVersionID, &editorAgentID, &editorVersionID, &testerLLMIDsJSON, &editorLLMID, &source); err != nil {
	...
	if strings.Join(testerLLMIDs, ",") != "claude-haiku-4-5,claude-opus-4-6" {
		t.Fatalf("tester_llm_ids = %#v", testerLLMIDs)
	}
	if editorLLMID != "claude-opus-4-6" {
