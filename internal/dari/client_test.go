...
func TestCreateSessionBatchSendsItems(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		item := got.Items[0]
		if item.AgentID != "agt_test" || item.LLMID != "claude-opus-4-6" || item.Metadata["kind"] != "tester" || len(item.Message.Content) != 1 {
			t.Fatalf("batch item = %#v", item)
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"batch_id":"batch_test","status":"queued","sessions":[{"index":0,"session_id":"sess_test","status":"queued","last_message_status":"queued","agent_id":"agt_test","version_id":"ver_test","llm_id":"claude-opus-4-6","metadata":{"kind":"tester"}}]}`))
	}))
	defer server.Close()
