...
Used by the `dari-docs` CLI as the fanout testing agent. The manifest exposes named LLM options (`claude-haiku-4-5`, `claude-sonnet-4-7`, `claude-opus-4-6`, `gpt-5-mini`, `gpt-5.1`, `gpt-5.5`) so the CLI runs each task under all bundled model tiers by default, or under an explicit repeated/comma-separated `--feedback-llm` matrix.
